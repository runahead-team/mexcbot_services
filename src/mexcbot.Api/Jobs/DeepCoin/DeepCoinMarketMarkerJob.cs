    ...
    public class DeepCoinMarketMarkerJob : BackgroundService
    {
        ...
        private async Task Run(BotDto bot)
        {
            ...
            try
            {
                ...
                {
                    if (exchangeInfo.Status != "live")
                    {
                        bot.Status = BotStatus.INACTIVE;
                        stopLog += $"Stop when exchange info is not live; \n";
                    }
                }
                ...
                {
                    var minQty = decimal.Parse(exchangeInfo.MinQty, new NumberFormatInfo());
                    ...
                    if (makerOption.MaxQty > maxLimitQty)
                    {
                        bot.Status = BotStatus.INACTIVE;
                        stopLog +=
